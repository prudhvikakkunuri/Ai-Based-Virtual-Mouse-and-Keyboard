...
window_name = "Virtual Mouse + Keyboard (Plane Tap)"
cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
cv2.resizeWindow(window_name, SCREEN_W, SCREEN_H)

def draw_input_box(img):
    cv2.rectangle(img, (50, 40), (SCREEN_W - 50, 120), (0, 255, 0),cv2.FILLED)
    cv2.putText(img, typed_text[-60:], (60, 90),
    ...
    if result.multi_hand_landmarks:
        for hand_landmarks in result.multi_hand_landmarks:
            ...
            if prev_iy is not None:
                if (iy - prev_iy) > 25:   # finger moving downward
                    press_key(ix, iy)
